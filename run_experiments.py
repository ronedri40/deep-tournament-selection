...
def build_graph_coloring(path, cfg, cross_prob, mut_prob):
    ...
    operators = [
        ...
        IntVectorOnePointMutation(probability=mut_prob,
...
def build_set_cover(path, cfg, cross_prob, mut_prob):
    ...
    operators = [
        VectorUniformCrossover(probability=cross_prob),
        BitStringVectorNFlipMutation(probability=mut_prob,
                                     probability_for_each=cfg.flip_mutation_prob,
                                     n=n),
    ]
...
def main():
    ...
    p.add_argument("--problems", nargs="+", default=list(PROBLEMS),
                   choices=list(PROBLEMS), help="which problems to run")
    ...
    quick_instance = {"tsp": ["att48.tsp"], "graph_coloring": ["queen8_12.col.txt"],
                      "set_cover": ["scp41.txt"]}
    dts_cfg = DTSConfig()
    ...
    for prob_name in args.problems:
        ...
        generations = (10 if args.quick else
                       (cfg.generations if args.full or args.generations is None else args.generations))
        if args.generations is not None and not args.full:
            generations = args.generations
        population_size = (20 if args.quick else
                           (args.population_size or cfg.population_size))
        ...
        for inst_name, inst_path in instances:
            for sel in selections:
                for run in range(runs):
                    ...
                    selection = make_selection(sel, population_size, vocab_size,
                                               dts_cfg=dts_cfg, device=args.device)
                    out = os.path.join(args.output, prob_name, inst_name, sel, f"run_{run}.json")
                    t0 = time.time()
                    ...
                    row = dict(problem=prob_name, instance=inst_name, size=size_label,
                               ...
                               **{spec["score_name"]: score}, seconds=round(dt, 1))
                    summary_rows.append(row)
                    print(f"[{prob_name:14s} {inst_name:22s} {sel:10s} run{run}] "
    ...
    if summary_rows:
        ...
        keys = ["problem", "instance", "size", "selection", "run",
                "best_fitness", "tour_length", "fitness", "seconds"]
