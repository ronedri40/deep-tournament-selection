"""Custom DTS reward used in the paper's Graph Coloring runs (Eliad's config).
...
0.25. Plugged into the operator via ``custom_reward_function``; the policy calls it
each generation as ``f(cur_gen_fitness, prev_gen_fitness, population)``.
"""
import numpy as np
...
def custom_reward(cur_gen_fitness_values: np.ndarray,
