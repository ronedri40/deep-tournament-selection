...
def main():
    ...
    p.add_argument("--instance", default=None, help="filename in data/set_cover, a path, or 'all'")
    ...
    for name, path in resolve_instances("set_cover", args.instance, cfg.instances):
        ...
        for run in range(args.runs):
            evaluator = SetCoverEvaluator(path, penalty=cfg.penalty, weighted=cfg.weighted)
            ...
            selection = make_selection(args.selection, args.population_size,
                                       vocab_size=2, dts_cfg=dts, device=args.device)
            out = os.path.join(args.output, "set_cover", name, args.selection, f"run_{run}.json")
            res = run_one(f"sc/{name}/{args.selection}/run{run}", creator,
